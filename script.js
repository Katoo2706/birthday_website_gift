...
document.querySelectorAll('.gift-box').forEach(box => {
    box.addEventListener('click', function() {
        if (!this.classList.contains('revealed')) {
            ...
            // Set the gift image source
            giftImage.src = giftSrc;

            // Reveal the gift with animation
            this.classList.add('revealed');

            // Optional: Disable other boxes after one is clicked
            document.querySelectorAll('.gift-box').forEach(otherBox => {
                if (otherBox !== this) {
